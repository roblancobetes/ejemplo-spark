...
object Main {
  ...
  def usoDatasets(): Unit = {
    ...
    val ds = spark.read
      ...
      .option("inferSchema", true)
      .csv("data/AAPL.csv")
      .withColumnRenamed("Adj Close", "adjClose") //Cambio el nombre para que coincida con la case class
      .as[Stock] //Utilizo el encoder (entendemos los datos como stocks)

    ds.show()

    val diasAlcistas = ds.filter(s => s.close > s.open)
